...
weighted <- union(which(substring(names(amb_filtered), nchar(names(amb_filtered)), nchar(names(amb_filtered))) == "m"), 
                  which(substring(names(amb_filtered), nchar(names(amb_filtered))-1, 
                                  nchar(names(amb_filtered))) %in% c("50", "90")))
...
c("A4","A5","A6","A16","A26","A29","A32","A35","A38","A52","A84","A87",
"A90","A93","A96","A99","A102","A105","A109","A114","A118","A122"),
denom=
c("A1","A1","A1","A13","A8","A9","A10","A11","A12","A49","A74","A75",
...
c("A0","A1","A2","A7","A8","A9","A10","A11","A12","A13","A14","A18","A19","A20","A21","A22","A23","A24","A27","A30",
...
c("A3","A15","A25","A28","A31","A34","A37","A51","A63","A66","A69","A72","A83","A86","A89","A92","A95","A98",
"A101","A104","A108","A117","A121"),
num =
c("A2","A14","A24","A27","A30","A33","A36","A50","A62","A65","A68","A71","A82","A85","A88","A91","A94","A97",
"A100","A103","A107","A116","A120"),
denom =
c("A1","A13","A8","A9","A10","A11","A12","A49","A58","A59","A60","A61","A74","A75","A76","A77","A78","A79",
...
amb_filtered[-regions, weightedlookup$denom[match(names(amb_filtered[-regions, weighted][-england,]), 
                                        		  weightedlookup$num)
								]][-england,][,x])}) 
...
for (i in (1:length(rc))) {
  ...
  sum(amb_filtered[k, meanlookup$num[
			match(names(amb_filtered[k, meaned]), 
                  meanlookup$metric)]][,x]) / sum(amb_filtered[k, meanlookup$denom[
			match(names(amb_filtered[k, meaned]), 
                  meanlookup$metric)]][,x])}
			) %>% 
			rnd() %>% 
  ...
  sum(amb_filtered[k, weighted][,x] * amb_filtered[k, weightedlookup$denom[match(names(amb_filtered[k, weighted]), 
...
RegionMapSingle <- RegionMap[-which(RegionMap$Region.Code %in% rc),][-england,]
SingleRegion <- amb_filtered[which(amb_filtered$Org.Code %in% RegionMapSingle$Org.Code),-c(1:4)] %>% arrange(Org.Name) %>% select(-1) == amb_filtered[which(amb_filtered$Org.Code %in% RegionMapSingle$Region.Code),-c(1:4)] %>% arrange(Org.Name) %>% select(-1)
